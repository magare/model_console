...
Provider CLIs emit different stdout formats. This module turns those provider-
specific streams into one shared `ProviderTrace` shape used by logs, transcript
viewing, and downstream JSON extraction.
"""
...
@dataclass
class ProviderTrace:
    """Normalized provider output used by transcripts, debugging, and JSON extraction."""
...
def extract_provider_trace(
    ...
    *,
    ...
    if provider_key == "copilot":
        return _extract_copilot_trace(stdout, final_text_override)
...
def _extract_copilot_trace(stdout: str, final_text_override: str | None) -> ProviderTrace:
    assistant_messages: list[str] = []
    reasoning: list[str] = []
    message_chunks: dict[str, list[str]] = {}
    reasoning_chunks: dict[str, list[str]] = {}
    stats: dict[str, Any] = {}
    ...
    for event in _iter_json_events(stdout):
        ...
        # Copilot emits both full messages and delta events. The parser accepts
        # either shape so prompt mode remains stable across CLI output variants.
        if event_type == "assistant.message":
            message_id = str(payload.get("messageId", ""))
            content = _coerce_string(payload.get("content")).strip()
            if message_id:
                message_chunks[message_id] = [content]
            if content:
                assistant_messages.append(content)

            reasoning_text = _coerce_string(payload.get("reasoningText")).strip()
            if reasoning_text:
                reasoning.append(reasoning_text)
        elif event_type == "assistant.message_delta":
            message_id = str(payload.get("messageId", ""))
            delta = _coerce_string(payload.get("deltaContent"))
            if message_id and delta:
                message_chunks.setdefault(message_id, []).append(delta)
        elif event_type == "assistant.reasoning":
            reasoning_id = str(payload.get("reasoningId", ""))
            content = _coerce_string(payload.get("content")).strip()
            if reasoning_id:
                reasoning_chunks[reasoning_id] = [content]
            if content:
                reasoning.append(content)
        elif event_type == "assistant.reasoning_delta":
            reasoning_id = str(payload.get("reasoningId", ""))
            delta = _coerce_string(payload.get("deltaContent"))
            if reasoning_id and delta:
                reasoning_chunks.setdefault(reasoning_id, []).append(delta)
    ...
    if not assistant_messages:
        assistant_messages = [
            message
            for message in (_join_event_chunks(chunks) for chunks in message_chunks.values())
            if message
        ]

    for content in (_join_event_chunks(chunks) for chunks in reasoning_chunks.values()):
        if content and content not in reasoning:
            reasoning.append(content)
...
def _extract_claude_trace(
    ...
    if isinstance(payload, dict) and str(payload.get("type", "")) == "result":
        ...
        final_text = final_text_override or result_text
        if not final_text and stderr.strip():
            final_text = stderr.strip()
        assistant_messages = [result_text] if result_text else ([final_text] if final_text else [])
        return ProviderTrace(
            provider="claude",
            raw_format="json",
            final_text=final_text,
...
def _iter_json_events(text: str) -> list[dict[str, Any]]:
    """Parse one-JSON-object-per-line streams while ignoring preambles/noise."""
    events: list[dict[str, Any]] = []
...
def _join_event_chunks(chunks: list[str]) -> str:
