...
from ..core.engine import LoopEngine
from ..paths import resolve_within_workspace
from ..observability.transcript_viewer import default_viewer_output_path, write_transcript_viewer
...
def cmd_run(args: argparse.Namespace) -> int:
    return _execute_loop(args, resume=False)


def cmd_resume(args: argparse.Namespace) -> int:
    return _execute_loop(args, resume=True)


def _execute_loop(args: argparse.Namespace, *, resume: bool) -> int:
    workspace = Path(args.workspace).resolve()
    runs_dir = resolve_within_workspace(workspace, args.runs_dir, "--runs-dir")
    loop_id, task_file, run_id = _resolve_engine_inputs(
        args=args,
        workspace=workspace,
        runs_dir=runs_dir,
        resume=resume,
    )
    config_dir = resolve_within_workspace(workspace, args.config_dir, "--config-dir")
    schemas_dir = resolve_within_workspace(workspace, args.schemas_dir, "--schemas-dir")
    prompts_dir = resolve_within_workspace(workspace, args.prompts_dir, "--prompts-dir")
    ...
    prune_summary = _prune_completed_runs(
        ...
        protected_run_ids={run_id or str(report.get("run_id", ""))},
    )
...
def cmd_transcript(args: argparse.Namespace) -> int:
    ...
    if args.transcript:
        transcript_path = resolve_within_workspace(workspace, args.transcript, "--transcript")
    else:
        run_dir = _resolve_run_dir(runs_dir, args.run_id)
    ...
    if args.output:
        output_path = resolve_within_workspace(workspace, args.output, "--output")
    else:
...
def _resolve_engine_inputs(
    *,
    ...
    if not resume:
        loop_id = str(args.loop)
        task_file = resolve_within_workspace(workspace, args.task, "--task")
        run_id = args.run_id
        return loop_id, task_file, run_id
    ...
    state = _load_json_object(state_file, "run state")
    loop_id = _require_non_empty_string(state, "loop_id", state_file)
    task_file = resolve_within_workspace(
        workspace,
        _require_non_empty_string(state, "task_file", state_file),
        "state.task_file",
    )
    return loop_id, task_file, args.run_id
...
def _load_json_object(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise FileNotFoundError(f"Could not read {label}: {path}") from exc
...
def _require_non_empty_string(payload: dict[str, Any], key: str, source: Path) -> str:
    value = payload.get(key)
    if isinstance(value, str) and value.strip():
        return value
    raise ValueError(f"Run state missing required field `{key}`: {source}")
...
def _event_printer(enabled: bool):
    ...
    def _print(event: dict[str, Any]) -> None:
        ...
        if formatter is None:
            return
        print(formatter(ts, event), flush=True)

    return _print


def _format_loop_started(ts: str, event: dict[str, Any]) -> str:
    return f"[{ts}] LOOP start run={event.get('run_id')} loop={event.get('loop_id')}"


def _format_roles_assigned(ts: str, event: dict[str, Any]) -> str:
    return (
        f"[{ts}] ROUND {event.get('round_id')} "
        f"impl={event.get('implementers')} rev={event.get('reviewers')}"
    )


def _format_model_command_started(ts: str, event: dict[str, Any]) -> str:
    return (
...
def _format_model_command_completed(ts: str, event: dict[str, Any]) -> str:
...
def _format_round_failed(ts: str, event: dict[str, Any]) -> str:
    return f"[{ts}] FAIL {event.get('round_id')} error={event.get('error')}"


def _format_loop_completed(ts: str, event: dict[str, Any]) -> str:
    return (
        f"[{ts}] LOOP done run={event.get('run_id')} "
        f"rounds={event.get('rounds_executed')} scores={event.get('scores')}"
    )


_EVENT_FORMATTERS = {
    "loop_started": _format_loop_started,
    "roles_assigned": _format_roles_assigned,
    "model_command_started": _format_model_command_started,
    "model_command_completed": _format_model_command_completed,
    "round_failed": _format_round_failed,
    "loop_completed": _format_loop_completed,
}
