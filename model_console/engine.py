...
class LoopEngine:
    ...
    def run(self) -> dict[str, Any]:
        ...
        while state["next_round_index"] < self.loop_cfg.max_rounds:
        ...
        if not state.get("terminated", False) and state["next_round_index"] >= self.loop_cfg.max_rounds:
            state["terminated"] = True
            self._save_state(state)
    ...
    def _run_round(
        ...
        task_text = self.task_file.read_text(encoding="utf-8")
